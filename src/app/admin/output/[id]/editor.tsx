...
import OutputChecker from '../insert/outputChecker'
import { set } from 'zod'
import { postOutput, putOutput } from '~/server/actions'
...
export default function EditorNostro({
    ...
    const [formData, setFormData] = useState({
        ...
        categories: formattedCategorie.map(c => ({
            ...
            options: c.options.map(option => {
                let value = false
                const found = formattedOutput.categories.find(cat => cat.id === c.id)
                if (found) {
                    const foundOption = found.options.find(opt => opt.name === option)
                    if (foundOption) {
    ...
    const handleSubmit = async() => {
        ...
        const result = await putOutput(formattedOutput.id, formData)
        if(result.success) {
            alert('Saved')
